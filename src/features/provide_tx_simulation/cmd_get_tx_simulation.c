...
tx_simulation_t G_transaction_check_info = {0};
...
static bool check_tx_simulation_validity(void) {
    ...
    if (G_transaction_check_info.risk == TRANSACTION_CHECK_RISK_UNKNOWN) {
        // No simulation data
...
void set_tx_simulation_warning(void) {
    ...
    if (!check_tx_simulation_params()) {
        ...
        G_transaction_check_info.risk = TRANSACTION_CHECK_RISK_UNKNOWN;
    }

    switch (G_transaction_check_info.risk) {
        case TRANSACTION_CHECK_RISK_UNKNOWN:
    ...
    warning.reportProvider = PIC(G_transaction_check_info.partner);
    warning.providerMessage = get_tx_simulation_category_str();
    warning.reportUrl = PIC(G_transaction_check_info.tiny_url);
}
