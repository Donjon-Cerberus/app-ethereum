...
#define DER_SIGNATURE_MIN_SIZE 8
...
static bool handle_risk(const tlv_data_t *data, tlv_extracted_t *tlv_extracted) {
    ...
    if (value >= TRANSACTION_CHECK_RISK_UNKNOWN) {
...
static tlv_transaction_check_status_t verify_struct(const tlv_extracted_t *tlv_extracted) {
    ...
    if (tlv_extracted->output->type == TRANSACTION_CHECK_TYPE_TRANSACTION) {
        if (!TLV_CHECK_RECEIVED_TAGS(tlv_extracted->received_tags, TAG_CHAIN_ID)) {
            PRINTF("Error: missing TAG_CHAIN_ID for TRANSACTION_CHECK_TYPE_TRANSACTION\n");
            return TLV_TRANSACTION_CHECK_MISSING_TAG;
        }
    }
    if (tlv_extracted->output->type == TRANSACTION_CHECK_TYPE_TYPED_DATA) {
        if (!TLV_CHECK_RECEIVED_TAGS(tlv_extracted->received_tags, TAG_DOMAIN_HASH)) {
            PRINTF("Error: missing TAG_DOMAIN_HASH for TRANSACTION_CHECK_TYPE_TYPED_DATA\n");
            return TLV_TRANSACTION_CHECK_MISSING_TAG;
        }
    }
...
tlv_transaction_check_status_t tlv_use_case_transaction_check(const buffer_t *payload,
    ...
    if (os_pki_get_info(&key_usage, trusted_name, &trusted_name_len, &public_key) == 0) {
        memmove(out->partner, trusted_name, TRANSACTION_CHECK_PARTNER_SIZE - 1);
    }
